...
contract NFTMarket is Ownable {
    ...
    // 事件定义
    event Listed(address indexed seller, address indexed nftContract, uint256 indexed tokenId);
    event Delisted(address indexed seller, address indexed nftContract, uint256 indexed tokenId);
    event Bought(address indexed buyer, address indexed nftContract, uint256 indexed tokenId);
    ...
    function list(address nftAddress, uint256 tokenId, address paymentToken, uint256 price) external {
        ...
        require(price > 0, "PRICE0");  // 保证上架价格大于0（这个就是paymentToken数量）
        ...
        emit Listed(msg.sender, nftAddress, tokenId);
    }

    // 卖家下架NFT
    function delist(address nftAddress, uint256 tokenId) external {
        Listing storage item = listings[nftAddress][tokenId];  // 得到该上架nft的信息
    ...
    function buyNFT(address nftAddress, uint256 tokenId) external {
        Listing storage listing = listings[nftAddress][tokenId];  // 得到该上架nft的信息
        require(listing.price > 0, "Not listed");  // 保证该nft已经上架

        // 买家支付 ERC20
        require(!(msg.sender == listing.seller), "cannot buy self");  // 保证该nft已经上架
        require(IERC20(listing.paymentToken).transferFrom(msg.sender, listing.seller, listing.price), "transfer failed");
